...
export default function Index() {
    ...
    async function loadData() {
      ...
      if (trendingData || moviesQueryData) {
        ...
        const heroSeries = (seriesQueryData && seriesQueryData.length > 0)
          ? [seriesQueryData[0]]
          : sortedTrending.filter((m: Movie) => m.type === 'series').slice(0, 1);
      ...
      try {
        ...
        // Use a hidden fetch for VJ extraction to keep logic unchanged but better performance
        const [moviesPage1, moviesPage2, seriesPage1] = await Promise.all([
          fetchMoviesSorted("movie", 100, 1),
          fetchMoviesSorted("movie", 100, 2),
          fetchSeries(100, 1),
        ]);

        const allMoviesData = [...moviesPage1, ...moviesPage2, ...seriesPage1];

        // Extract unique VJs from all movies and series
        const vjSet = new Set<string>();

        // Always include Top/Popular VJs
  ...
  const handleVJChange = useCallback(async (vj: string | null) => {
    ...
    try {
      ...
      const data = await fetchMoviesSorted("movie", 100, 1);
    ...
    try {
      ...
      if (filters.contentType) {
        ...
        if (filters.contentType === "movies") {
          data = await fetchMoviesSorted("movie", 100, 1);
        } else {
  ...
  const handleLoadMore = useCallback(async () => {
    ...
    try {
      ...
      let more: Movie[] = [];
      if (viewMode === "movies") {
        more = await fetchMoviesSorted("movie", 20, nextPage);
